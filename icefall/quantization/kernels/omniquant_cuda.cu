...
__global__ void omniquant_kernel(
    ...
    const acc_t q_plus_eps = q_val + 0.5;
    for (index_t i = idx; i < size; i += stride) {
        acc_t x_val = static_cast<acc_t>(x[i]);

        acc_t div = x_val / s_val;
        acc_t rounded = roundf(div);
        acc_t clamped = (rounded < -q_plus_eps)
            ? -q_val
            : (rounded > q_plus_eps) ? q_val : rounded;
        uint8_t mask_val = (rounded >= -q_plus_eps) && (rounded <= q_plus_eps);

        acc_t dx_ds_val = mask_val ? clamped - div : clamped;
...
std::vector<at::Tensor> omniquant_cuda(
    at::Tensor x,
    at::Tensor scale,
    const float q,
    const bool inplace
    ...
    AT_DISPATCH_FLOATING_TYPES_AND2(
        ...
        ([&] {
            if (at::cuda::detail::canUse32BitIndexMath(x)) {
                omniquant_kernel<scalar_t, int32_t><<<blocks, threads>>>(
                    x.data_ptr<scalar_t>(),
                ...
                omniquant_kernel<scalar_t, int64_t><<<blocks, threads>>>(
                    x.data_ptr<scalar_t>(),
                    scale.data_ptr<float>(),
                    out.data_ptr<scalar_t>(),
                    dx_ds.data_ptr<scalar_t>(),
