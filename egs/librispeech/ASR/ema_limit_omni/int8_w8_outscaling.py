...
class QModule(torch.nn.Module):
    def __init__(
        ...
        self.module.weight.data.copy_(weight.round().to(self.dtype))
    ...
    def forward(self, x: torch.Tensor):
        if self.mode == "observe":
            ...
            w = w.double()
            b = self.bias
            x = self.forward_module(x.double(), w, None).to(self.dtype) * (scale_w * self.scale_out)
            x = self.addbias(x, b)
            return x
...
def insert_observers(
    ...
    *args, **kwargs
    ...
    def replacement_fn_linear(m):
        weight_obs = OBSERVER_DICT[weight_observer](dtype_weight, dtype_scale)
