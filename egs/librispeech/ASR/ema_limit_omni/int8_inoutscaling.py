...
class QModule(nn.Module):
    def __init__(
        ...
        weight_obs_scaled: AffineQuantizedObserverBase,
        alpha: float = 0.5,
    ):
        super().__init__()
        self.module = module
    ...
    def set_quantized_mode(self):
        ...
        scale_input = q(1 / self.reshape_act_scale(act_scale), self.dtype_scale, True, True)
        self.register_buffer("scale_input", scale_input)
        self.scale_input: Tensor
        ...
        # get weight_scale
        weight_scale = weight.abs().view(weight.size(0), -1).max(dim=1).values / self.qmax_weight
        weight /= self.reshape_weight_scale(weight_scale, "cout")
        weight = weight.round().to(self.dtype)
        self.register_buffer("weight", q(weight, self.dtype))
        self.weight: Tensor

        scale_out = self.reshape_act_scale(weight_scale)
        scale_out = q(scale_out, self.dtype_scale, True, True)
        self.register_buffer("scale_output", scale_out)
        self.scale_output: Tensor

        self.register_buffer("bias", self.module.bias)
    ...
    def forward(self, x: Tensor):
        if self.mode == "quantized":
            x = q(x * self.scale_input, self.dtype)
            x = x.clamp(min=-self.qmax_act, max=self.qmax_act).round().double()
            w = self.weight.double()
            x = q(self.forward_module(x, w, None), self.dtype) * self.scale_output
            x = q(x, self.dtype)
...
class QEmbedding(nn.Module):
    def __init__(self, module, weight_obs, dtype_target: torch.dtype):
        ...
        weight = weight.tensor_impl.get_plain()[0].to(self.dtype)
        self.register_buffer("weight", weight)
        self.weight: Tensor
        self.register_buffer("scale_output", weight_scale)
        self.scale_output: Tensor

    def forward(self, x):
        x = q(F.embedding(x, self.weight), self.dtype) * self.scale_output
...
def insert_observers(
    ...
    *args, **kwargs,
    ...
    def replacement_fn_linear(m):
        act_obs = OBSERVER_DICT[act_observer](dtype_act, dtype_scale, PerAxis(axis=-1)) # 
        weight_obs = OBSERVER_DICT[weight_observer](dtype_weight, dtype_scale, PerAxis(axis=1))
        weight_obs_scaled = OBSERVER_DICT[weight_observer](dtype_weight, dtype_scale, PerTensor())
        return QLinear(m, act_obs, weight_obs, weight_obs_scaled, alpha=alpha)

    def replacement_fn_dwconv(m):
        act_obs = OBSERVER_DICT[act_observer](dtype_act, dtype_scale, PerAxis(axis=1))  # 
        weight_obs = OBSERVER_DICT[weight_observer](dtype_weight, dtype_scale, PerAxis(axis=0))
        weight_obs_scaled = OBSERVER_DICT[weight_observer](dtype_weight, dtype_scale, PerTensor())
