...
def add_model_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        ...
        type=str,
        default='BatchNorm',
        help="Encoder normalization layer.",
    )
...
def get_joiner_model(params: AttributeDict) -> nn.Module:
    joiner = Joiner(
        ...
        vocab_size=params.vocab_size,
        weight_norm=params.weight_norm,
        logit_no_bias=params.logit_no_bias,
    )
    return joiner
...
def run(rank, world_size, args):
    ...
    optim_groups = {
        "regex_list": [
            "encoder\\.conv_pre.+weight\\.original0",
            "encoder\\.cnn\\.\d+\\.scale",
            "ema\\.weight",
            "encoder.+se\\.sequential\\.3\\.bias",  # Right before tanh
            "encoder.+se\\.sequential\\.3\\.parametrizations\\.weight\\.original0",  # Right before tanh
            "encoder.+activation\\.weight",
            "encoder\\.proj\\.parametrizations\\.weight\\.original0",
            "decoder\\.embedding",
            "(decoder|joiner).+(parametrizations\\.weight\\.original0|bias)",
            # Below -> ignore
            # "joiner.+parametrizations\\.weight\\.original0",
            # "simple_(am|lm)_proj\\.(parametrizations\\.weight\\.original0|bias)", -> Bad performance
        ],
        "weight_decay": 0.0,
    }
    model_params = update_param_groups(model, [optim_groups])
    ...
    elif params.optimizer_name == "AdamP":
        optimizer = AdamP(
            model_params,
            lr=params.initial_lr,
            weight_decay=params.weight_decay,
            weight_decay_projection=params.weight_decay_projection,
            betas=(params.beta1, params.beta2),
        )
    elif params.optimizer_name == "Eve":
        optimizer = Eve(model_params, lr=params.initial_lr, weight_decay=params.weight_decay)
    else:
    ...
    if params.data_reazonspeech_train:
        train_cuts += datamodule.reazonspeech_train_cuts()
    ...
    if params.data_reazonspeech_dev:
        valid_cuts += datamodule.reazonspeech_dev_cuts()
    validate(valid_cuts)
