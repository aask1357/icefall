...
class Conv1d(nn.Conv1d):
    def __init__(self, *args, n_bits: int = 8, eps: float = 0.0, **kwargs):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        y = F.conv1d(x, weight, self.bias, self.stride, self.padding,
                     self.dilation, self.groups)
        return y


class CausalConv1d(nn.Conv1d):
    def __init__(self, *args, use_cache=False, n_bits: int = 8, eps: float = 0.0, **kwargs):
        super().__init__(*args, **kwargs)
...
class CausalSE(nn.Module):
    ...
    def __init__(
        ...
        self.sequential = nn.Sequential(
            ...
            ScaledConv1d(dim, dim // 8, 1, n_bits=n_bits_weight, eps=eps),
            Act(inplace=True),
            get_activation_quantizer(clamp_method, n_bits_act, eps, act_decay=act_decay),
            ScaledConv1d(dim // 8, dim, 1, n_bits=n_bits_weight, eps=eps),
        )
        if se_gate == "sigmoid":
...
class ConvBlock(nn.Module):
    def __init__(
        ...
        self.norm0 = Norm(channels, affine=False) if norm0 else nn.Identity()
        self.clamp0 = get_activation_quantizer(clamp_method, n_bits_act, eps, act_decay=act_decay)
        self.pointwise1 = ScaledConv1d(
...
class Conv1dSubsampling(nn.Module):
    def __init__(self, in_ch, out_ch, subsampling_factor, norm,
        ...
        self.conv = nn.Sequential(
            ...
            get_activation_quantizer(clamp_method, n_bits_act, eps, act_decay=act_decay),
            ScaledConv1d(
                out_ch, out_ch, 2*sf, stride=sf, padding=0, groups=out_ch, bias=bias,
                learn_weight_scale=False, n_bits=n_bits_weight, eps=eps, gamma=conv_scale_gamma,
            ),
...
class Encoder(EncoderInterface):
    def __init__(
        ...
        self.proj = nn.Sequential(
            ...
            get_activation_quantizer(clamp_method, n_bits_act, eps, act_decay=act_decay),
            ScaledConv1d(channels, output_channels, 1, bias=False),
        )
