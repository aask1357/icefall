...
def add_model_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        ...
        type=str,
        default='ReLU',
        help="Encoder activation.",
    )

    parser.add_argument(
        "--encoder-se-activation",
        type=str,
        default='ReLU',
        help="Encoder squeeze-and-excitation layers' activation.",
    )
...
def get_encoder_model(params: AttributeDict) -> nn.Module:
    ...
    encoder = Encoder(
        ...
        clamp_method=params.clamp_method,
        skip=params.skip,
        n_bits_weight=params.n_bits_weight,
        n_bits_act=params.n_bits_act,
        act_decay=params.act_decay,
        eps=params.eps,
        conv_scale_gamma=params.conv_scale_gamma,
        ema_normalize=params.ema_normalize,
        manorm_after_ema=params.manorm_after_ema,
    )
    return encoder
...
def compute_loss(
    ...
    with torch.set_grad_enabled(is_training):
        ...
        if not torch.all(is_finite):
            ...
            torch.save(model.state_dict(), params.exp_dir / f"epoch-{params.cur_epoch}-{rank}-failed.pt")
...
def train_one_epoch(
    ...
    Args:
      ...
      valid_dl_dict:
        Dictionary of dataloaders for the validation dataset.
      scaler:
        The scaler used for mix precision training.
      model_avg:
        The stored model averaged from the start of training.
            ...
            for name, valid_dl in valid_dl_dict.items():
                ...
                if tb_writer is not None:
                    valid_info.write_summary(
                        tb_writer, f"train/valid_{name}_", params.batch_idx_train
                    )
...
def run(rank, world_size, args):
    ...
    channelwise = {
        "regex_list": [
            "encoder\\.conv_pre\\.conv\\.[13]\\.weight$",
            "encoder\\.cnn\\.\\d+\\.pointwise[12]\\.weight$",
            "encoder\\.cnn\\.\\d+\\.depthwise\\.weight$",
            "encoder\\.cnn\\.\\d+\\.se\\.sequential\\.[14]\\.weight$",
            "encoder\\.proj\\.2\\.weight$",
            "decoder\\.embedding\\.weight$",
        ],
        "channelwise": True,
    }
    scale = {
        "regex_list": ["encoder\\.cnn\\.\\d+\\.scale$"],
        "channelwise": True,
        "target_rms": 1.0,
    }
    param_groups = [no_wd, bias, channelwise]
    model_params = update_param_groups(model, param_groups)
    ...
    def remove_short_and_long_utt(c: Cut):
        ...
        # the threshold
        if c.duration < 0.5 or c.duration > 35.0:
            logging.warning(
    ...
    valid_cuts_dict = dict()
    if params.data_libri_dev_clean:
        valid_cuts_dict["libri-dev-clean"] = datamodule.dev_clean_cuts()
    if params.data_libri_dev_other:
        valid_cuts_dict["libri-dev-other"] = datamodule.dev_other_cuts()
