...
class ScaledBatchNorm1d(nn.BatchNorm1d):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        if self.affine:
            self.weight.data.fill_(0.1)
            self.weight_scale = nn.Parameter(torch.log(torch.ones(1) * 10))
...
class ScaledSyncBatchNorm(nn.SyncBatchNorm):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        if self.affine:
            self.weight.data.fill_(0.1)
...
class ScaledLinear(nn.Linear):
    ...
    def get_weight(self):
        ...
        if self.q_max is not None:
            weight = quantize(self.weight, self.q_max, self.eps)
        return weight * self.weight_scale.exp()
...
def get_activation(activation: str, activation_kwargs: dict = {}) -> nn.Module:
    if activation == "TanhX":
        return TanhX()
    else:
        return getattr(nn, activation)(**activation_kwargs)


class ScaledConv1dAct(nn.Conv1d):
    def __init__(
        self,
        *args,
        initial_scale: float = 1.0,
        initial_speed: float = 1.0,
        learn_weight_scale: bool = True,
        n_bits: Optional[int] = None,
        eps: float = 1.0e-8,
        gamma: float = 1.0,
        ...
        **kwargs,
        ...
        if learn_weight_scale:
            assert gamma == 1.0, "Cannot use both learn_weight_scale and gamma"
        else:
            assert 0.0 < gamma <= 1.0, "gamma must be in (0, 1] if not learn_weight_scale"
        super().__init__(*args, **kwargs)
        self.eps = eps
        self.gamma = gamma
        initial_scale = torch.tensor(initial_scale).log()
        ...
        if learn_weight_scale:
            ...
            self.register_buffer(
                "weight_scale", initial_scale.exp().repeat(self.out_channels).view(-1, 1, 1)
            )
        ...
        self.q_max = None if n_bits is None else float(2 ** (n_bits - 1) - 1)
        self.eps = eps
        self.act = get_activation(activation, activation_kwargs)
        self.target = target
        self.register_buffer("weight_scale_prev", torch.ones(self.out_channels))
        self.weight_scale_prev: Tensor
        self.register_buffer("std", torch.ones(self.out_channels))
        self.std: Tensor
    ...
    def _reset_parameters(self, initial_speed: float):
        ...
        if self.learn_weight_scale:
            self.weight_scale.data += torch.tensor(scale / std).log()
        elif self.gamma == 1.0:
            self.weight_scale.data *= scale / std
        else:
            scale = self.weight_scale.data * (scale / std)
            self.weight_scale.data.copy_(1 / scale)
    ...
    def get_weight(self):
        ...
        if self.learn_weight_scale:
            ...
            return weight / self.weight_scale.clamp_min(self.eps)
    ...
    def forward(self, input: Tensor) -> Tensor:
        ...
        if self.gamma == 1.0 or not self.training:
            return self.act(x)
        with torch.no_grad():
            if self.target == "std":
                std_scaled = torch.std(x, dim=(0,2)).view(-1, 1, 1)
                target = std_scaled * self.weight_scale
            elif self.target == "scale":
                scale_scaled = x.norm(dim=(0, 2)).mul(x[:, 0, :].numel()**-0.5).view(-1, 1, 1)
                target  = scale_scaled * self.weight_scale
            scale_prev = self.weight_scale.data.view(-1).clone()
            self.weight_scale.data.mul_(self.gamma).add_((1.0 - self.gamma) * target)
            self.std.data.copy_(target.view(-1))
            self.weight_scale_prev.data.copy_(scale_prev.view(-1))
            if ~torch.all(torch.isfinite(self.weight_scale.data)):
                self.register_buffer("input", input.data)
                self.register_buffer("weight_calculated", w.data)
                self.register_buffer("output", x.data)
        x = self.act(x)
    ...
    def forward(self, x: Tensor) -> Tensor:
        if self.training:
            with torch.no_grad():
                ...
                self.scale.data.mul_(self.gamma).add_((1.0 - self.gamma) * std)
        scale = self.scale.view(-1, 1).clamp_min(self.eps)
        return x.div_(scale) if self.inplace else x.div(scale)
