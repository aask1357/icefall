...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=str2bool,
        default=True,
        help="Whether to update BatchNorm stastistics using train dataset. "
        "Used only when --use-averaged-model is True and --encoder-norm is "
        "BatchNorm"
    )
...
def decode_one_batch(
    ...
    # def plot_mel(x):
    #     plt.clf()
    #     im = plt.imshow(x[0].detach().cpu().numpy(),
    ...
    # with torch.no_grad():
    #     # conv_pre
    #     a = (feature.transpose(1, 2)[0:1, :, :] - model.encoder.mean) * model.encoder.rstd
    #     # b = model.encoder.conv_pre.pointwise(a)
    #     # c = model.encoder.conv_pre.depthwise(b)
    #     # norm = model.encoder.conv_pre.norm
    #     # d = (c - norm.running_mean.view(1, -1 ,1)) / norm.running_var.sqrt().view(1, -1, 1)
    #     # e = d * norm.weight.data.view(1, -1, 1) + norm.bias.data.view(1, -1, 1)
    #     y, lengths = model.encoder.conv_pre(a, feature_lens)

    #     # blocks
    #     y_in = y
    #     for block in model.encoder.cnn[:0]:
    #         y, lengths = block(y, lengths, 1.0)   # [batch_size, channels, time]

    #     # conv_post
    #     y = model.encoder.proj(torch.cat((y, y_in), dim=1))
...
def main():
    ...
    # Extract feature
    from local.enhanced_fbank import CustomFbank, CustomFbankConfig
    extractor = CustomFbankConfig()
    wav, _ = librosa.load("exp/ko/008_laud_babble100.wav", sr=16000)
    x = torch.tensor(wav, device=device).unsqueeze(0)
    x = torch.nn.functional.pad(x, (0, 160*30))
    feature = extractor.extract_batch(x)
    feature_lens = torch.tensor([feature.size(1)], device=device)
