...
class QModule(torch.nn.Module):
    ...
    def set_scaled_mode(self):
        ...
        # Compute scale
        scale = act_max.squeeze().clamp_min(1e-5)
        ...
        shape = [weight.size(0)] + [1] * (weight.dim() - 1)
        weight_max = weight.abs().view(weight.size(0), -1).max(dim=1).values.clamp_min(1e-5)
        weight = weight / weight_max.view(shape)
        self.scale_out = weight_max.squeeze()

        if self.act_obs.granularity.axis == 1:
            # Conv, DWConv
            self.scale_out = self.scale_out.view(-1, 1)
    ...
    def forward(self, x: torch.Tensor):
        ...
        x = self.forward_module(x, w, None) * self.scale_out
