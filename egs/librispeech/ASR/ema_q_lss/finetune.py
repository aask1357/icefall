...
def get_transducer_model(params: AttributeDict) -> nn.Module:
    ...
    model = Transducer(
        ...
        joiner_dim=params.joiner_dim,
        vocab_size=params.vocab_size,
        detach_simple_am=params.detach_simple_am,
        detach_simple_lm=params.detach_simple_lm,
    )
    return model
...
def run(rank, world_size, args):
    ...
    if params.start_epoch == 1:
        ...
        if not params.use_averaged_model:
            if params.avg == 1:
                ...
                start = params.load_epoch - params.avg + 1
                filenames = []
                for i in range(start, params.load_epoch + 1):
                    if i >= 1:
                        filenames.append(f"{params.load_dir}/epoch-{i}.pt")
                logging.info(f"averaging {filenames}")
                model.load_state_dict(average_checkpoints(filenames, device=device))
        else:
            assert params.avg > 0, params.avg
            start = params.load_epoch - params.avg
            assert start >= 1, start
            filename_start = f"{params.load_dir}/epoch-{start}.pt"
            filename_end = f"{params.load_dir}/epoch-{params.load_epoch}.pt"
            logging.info(
                f"Calculating the averaged model over epoch range from "
                f"{start} (excluded) to {params.load_epoch}"
            )
    ...
    if params.quantizer_gamma_lr_ratio != 1.0 and not params.fix_quantizer:
        q_gamma = dict(
            regex_list = [
                "\\.quantizer_(act|weight)\\.gamma$",
            ],
