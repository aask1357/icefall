...
class NormConv1d(nn.Conv1d):
    def __init__(self, *args, affine: bool = True, **kwargs):
    ...
    def __init__(
        ...
        if act_bal:
            self.sequential = nn.Sequential(
                ScaledConv1d(dim, dim // 8, 1),
                ActivationBalancer(1),
                Act(inplace=False),
                ScaledConv1d(dim // 8, dim, 1),
                ActivationBalancer(1),
            )
        else:
            self.sequential = nn.Sequential(
...
class Conv1dSubsampling(nn.Module):
    def __init__(self, in_ch, out_ch, subsampling_factor,
        ...
        if weight_norm:
            depthwise = Conv1d(out_ch, out_ch, 2*sf, stride=sf, padding=0,
                               groups=out_ch, bias=True)
            self.depthwise = weight_norm_fn(depthwise)
        else:
            self.depthwise = ScaledConv1d(out_ch, out_ch, 2*sf, stride=sf, padding=0,
                                          groups=out_ch, bias=True)
...
class Encoder(EncoderInterface):
    def __init__(
        ...
        weight_norm: bool = False, # not used.
