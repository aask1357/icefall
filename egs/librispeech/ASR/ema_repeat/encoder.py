...
class ConvBlock(nn.Module):
    def __init__(
        ...
        self.pre_norm = Norm(channels, affine=False) if block_pre_norm else nn.Identity()
        ...
        self.norm1 = Norm(channels_hidden, affine=False) if shared_norm else nn.ModuleList(
            Norm(channels_hidden, affine=False) for _ in range(repeat)
        )
        self.depthwise = CausalConv(
            channels_hidden, channels_hidden, kernel_size, groups=channels_hidden,
            use_cache=use_cache)
        self.norm2 = Norm(channels_hidden, affine=False) if shared_norm else nn.ModuleList(
            Norm(channels_hidden, affine=False) for _ in range(repeat)
        )
    ...
    def forward(
        ...
        for i in range(self.repeat):
            norm1 = self.norm1[i] if isinstance(self.norm1, nn.ModuleList) else self.norm1
            norm2 = self.norm2[i] if isinstance(self.norm2, nn.ModuleList) else self.norm2
            x = self.pointwise1(x)
            x = self.act_bal(x)
            x = self.activation(x)
            x = norm1(x)
...
class Encoder(EncoderInterface):
    def __init__(
        ...
        for _ in range(num_blocks):
            layer = ConvBlock(
                channels, channels_expansion, kernel_size,
                ...
                repeat=repeat, shared_norm=shared_norm, block_pre_norm=block_pre_norm,
            )
