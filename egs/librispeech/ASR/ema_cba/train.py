...
def get_joiner_model(params: AttributeDict) -> nn.Module:
    joiner = Joiner(
        ...
        vocab_size=params.vocab_size,
        dec_residual=params.dec_residual,
        weight_norm=params.weight_norm,
    )
    return joiner
...
def run(rank, world_size, args):
    ...
    no_wd = {
        "regex_list": ["ema\\.weight", "scale"], #, "parametrizations\\.weight\\.original0", "bias", "norm.+weight"],
        "weight_decay": 0.0
    }
    param_groups, param_names = update_param_groups(model, [no_wd], verbose=(rank == 0))
    if params.optimizer_name == "AdamP":
        optimizer = AdamP(
            param_groups,
            lr=params.initial_lr,
            betas=(params.beta1, paams.beta2),
    ...
    elif param.optimizer_name == "ScaledAdam":
        optimizer = ScaledAdam(
            param_groups,
            lr=params.initial_lr,
            betas=(params.beta1, paams.beta2),
            parameters_names=param_names
        )
    else:
