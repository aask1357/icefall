    ...
    def __init__(
        ...
        if act_bal:
            self.sequential = nn.Sequential(
                Conv(dim, dim // 8, 1),
                ActivationBalancer(dim // 8, channel_dim=1, max_abs=10.0,
                                   min_positive=0.05, max_positive=1.0),
                Act(inplace=False),
                Conv(dim // 8, dim, 1),
                ActivationBalancer(dim, channel_dim=1, max_abs=10.0,
                                   min_positive=0.05, max_positive=1.0),
            )
        else:
            self.sequential = nn.Sequential(
...
class ConvBlock(nn.Module):
    def __init__(
        ...
        self.pointwise2 = Conv(channels_hidden, channels, 1, bias=bias)
        ...
        channels_scale = 1 if single_scale else channels
        if zero_init_residual:
            self.scale = nn.Parameter(torch.zeros(1, channels_scale, 1))
        else:
            self.scale = nn.Parameter(torch.ones(1, channels_scale, 1))
    ...
    def remove_weight_reparameterizations(self):
        ...
        if self.norm in ["BatchNorm", "SyncBatchNorm"]:
            ...
            self.pointwise1 = nn.Conv1d(channels, channels_hidden, 1, bias=True, **kwargs)
    ...
    def forward(
        ...
        with torch.no_grad():
            self.scale.clamp_(min=-self.scale_limit, max=self.scale_limit)
...
class Conv1dSubsampling(nn.Module):
    def __init__(self, in_ch, out_ch, subsampling_factor,
        ...
        self.pointwise = Conv(in_ch, out_ch, 1, bias=False)
        self.depthwise = Conv(out_ch, out_ch, 2*sf, stride=sf, padding=0,
...
class Encoder(EncoderInterface):
    def __init__(
        ...
        for dilation in dilations:
            layer = ConvBlock(
                ...
                scaled_conv, act_bal, whitener, zero_init_residual, se_gate,
                gamma=gamma, use_cache=use_cache, chunksize=chunksize,
                scale_limit=scale_limit, single_scale=single_scale, weight_norm=weight_norm,
            )
        ...
        self.proj = Conv(channels, output_channels, 1, bias=bias)
    ...
    def remove_weight_reparameterizations(self):
        if self.scaled_conv:
            ...
            self.proj.weight.data.copy_(q(proj.get_weight()))
