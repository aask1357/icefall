...
class WeightQuantizer(nn.Module):
    def __init__(
        ...
        super().__init__()
        assert mode in ["scale", "emamax", "max"]
        self.q_max = float(2 ** (n_bits - 1) - 1)
        self.eps = eps
        self.decay = decay
        self.mode = mode
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if not self._is_initialized_cpu:
            ...
            if not self._is_initialized_cpu:
                if self.mode == "scale":
                    scale = x.detach().abs().amax() / self.q_max
                    self.gamma.data.fill_(scale)
                elif self.mode == "emamax":
                    self.x_abs_max.data.fill_(x.detach().abs().amax())
                self.is_initialized.data.fill_(True)
                self._is_initialized_cpu = True

        if self.mode == "scale":
            scale_max = x.detach().abs().amax() / self.q_max
            ...
            with torch.autocast("cuda", enabled=False):
                if self.mode == "emamax":
                    if self.training:
                        ...
                        self.x_abs_max.data.mul_(self.decay).add_(x_abs_max * (1 - self.decay))
