...
class CustomFbank(TorchaudioFbank):
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.ft_weight = Q((window * ft_weight)).to(self.config.device)
        ...
        self.mel_fbank = Q(mel_fbank).to(self.config.device)

        self.resample = Resample(
            orig_freq=self.config.sampling_rate_orig,
    ...
    def extract(self, samples, sampling_rate=None):
        ...
        y = Q(torch.from_numpy(samples)).view(-1).to(self.device)
    ...
    def extract_batch(self, samples, sampling_rate=None, lengths=None):
        ...
        y = _get_strided_batch(y, self.win_size, self.hop_size, self.config.snip_edges)   # [B, T_mel, win_size]
        
        # remove DC
        if self.config.remove_dc_offset:
            row_means = Q(torch.mean(y, dim=2, keepdim=True))
            y = Q(y - row_means)
        
        # pre-emphasis
        if self.config.preemphasis_coefficient != 0.0:
            y_prev = F.pad(y, (1, 0), mode="replicate")
            y = Q(y - self.config.preemphasis_coefficient * y_prev[:, :, :-1])

        # window + stft
        y = Q(F.linear(y.float(), self.ft_weight, None))    # [B, T_mel, n_fft] (real: n_fft//2, imag: n_fft//2 -> concatenated)
